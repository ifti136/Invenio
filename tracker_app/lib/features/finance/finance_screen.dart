...
Future<List<RuleFinanceData>> financeData(FinanceDataRef ref) async {
  ...
  return rules.map((rule) {
    final fin = financials[rule.id] ?? RuleFinancials(
      accumulatedProfit: 0,
      totalSpent: 0,
      availableBalance: 0,
    );
...
class FinanceScreen extends ConsumerWidget {
    ...
    return Scaffold(
      ...
      body: dataAsync.when(
        ...
        data: (data) {
          if (data.isEmpty) {
            return const Center(
              child: Text('No allocation rules found. Add some in settings.', 
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Row(
                            ...
                            children: [
                              _buildFinanceStat(context, 'Accumulated', fin.accumulatedProfit),
                              _buildFinanceStat(context, 'Spent', fin.totalSpent),
                              _buildFinanceStat(
                                context, 
                                'Available', 
                                fin.availableBalance, 
                                color: fin.availableBalance >= 0 ? AppColors.teal : Colors.redAccent,
                              ),
                            ],
                          ),
                        ],
  ...
  Widget _buildFinanceStat(BuildContext context, String label, double value, {Color? color}) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(
