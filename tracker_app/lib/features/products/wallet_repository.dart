...
class WalletBalance {
  ...
  WalletBalance({required this.walletId, required this.name, required this.balance});
}
...
class WalletRepository {
  ...
  Future<List<WalletBalance>> getWalletBalances() async {
    ...
    final salesQuery = await _db.customSelect(
    ...
    final expensesQuery = await _db.customSelect(
  ...
  Future<bool> updateWallet(int id, String name, String type, double openingBalance, bool isActive) async {
    return await (_db.update(_db.wallets)..where((t) => t.id.equals(id))).replace(
      WalletsCompanion(
        name: drift.Value(name),
        type: drift.Value(type),
        openingBalance: drift.Value(openingBalance),
  ...
  Future<int?> getLastUsedWalletId() async {
    final lastSale = await (_db.select(_db.sales)..orderBy([(t) => drift.OrderingTerm.desc(t.date)])).get();
    final lastExpense = await (_db.select(_db.expenses)..orderBy([(t) => drift.OrderingTerm.desc(t.date)])).get();
