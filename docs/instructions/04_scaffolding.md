...
> **Status:** As-shipped at v1.0.0+2 (post-Phase 7.0). Dep versions
> match `tracker_app/pubspec.yaml`; `schemaVersion` is **2** with
> `onUpgrade` migration. Includes `glass_kit` + `aurora_background`
> + `flutter_launcher_icons`. For the human-readable architecture
...
### Step 4b — Configure launcher icon (Phase 7.0)

If you want a custom launcher icon, drop your `invenio.png` (or
similar, ideally 1024×1024+) into `tracker_app/assets/icon/` and
add the following block to `pubspec.yaml`:

```yaml
flutter_launcher_icons:
  android: true
  image_path: assets/icon/invenio.png
  adaptive_icon_background: '#1D9E75'
  adaptive_icon_foreground: assets/icon/invenio.png
  min_sdk_android: 24
```

Then run `dart run flutter_launcher_icons` to generate the 5
mipmap variants + adaptive icon descriptors.
